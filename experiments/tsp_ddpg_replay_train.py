...
from moco.replay_buffer import create_replay_buffer, add_batch as rb_add_batch, sample_batch as rb_sample_batch
import mlflow
...
if __name__ == "__main__":
    ...
    parser.add_argument("--replay_min_size", type=int, default=8192)
    ...
    def collect_episodes_batched(task_family,
        ...
        Collect E episodes of length T=max_length in parallel and return a flat batch:
          ...
          senders_rel, receivers_rel   (per-graph topology; relative indices 0..N-1)
        ...
        return (states_flat, actions_flat, rewards_flat, next_states_flat, dones_flat, timesteps, senders_ETe, receivers_ETe)
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id) as mlflow_cm, disable_jit(args.disable_jit) as jit_cm:
        ...
        # Initialize replay buffer lazily on first collected batch
        replay_buffer = None
        ...
        for i in tqdm.tqdm(range(args.outer_train_steps), disable=args.disable_tqdm):
            ...
            # Unpack and optionally initialize replay buffer
            (states_flat, actions_flat, rewards_flat, next_states_flat, dones_flat, timesteps, senders_rel, receivers_rel) = batch

            if replay_buffer is None:
                num_nodes = int(next_states_flat.n_node[0])
                num_edges = int(next_states_flat.n_edge[0])
                node_feat = int(states_flat.nodes.shape[-1])
                edge_feat = int(states_flat.edges.shape[-1])
                glob_feat = int(states_flat.globals.shape[-1])
            ...
            # Choose training batch: sample from replay if sufficiently filled; else use fresh
            # RNG for replay sampling
            key, sample_key = jax.random.split(key)
            use_replay = int(jax.device_get(replay_buffer.size)) >= args.replay_min_size
            def _train_with_replay():
                skey = jax.random.fold_in(sample_key, i)  # per-step derived key
                rb_states, rb_actions, rb_rewards, rb_next_states, rb_dones = rb_sample_batch(
                    replay_buffer, skey, args.replay_batch_size
                )
                return ddpg_agent.update(
                    ddpg_state,
                    (rb_states, rb_actions, rb_rewards, rb_next_states, rb_dones),
                    train_actor=train_actor,
                    train_critic=train_critic,
                )
            def _train_with_fresh():
                return ddpg_agent.update(
                    ddpg_state,
            ...
            if use_replay:
                ddpg_state, metrics = _train_with_replay()
            else:
                ddpg_state, metrics = _train_with_fresh()
