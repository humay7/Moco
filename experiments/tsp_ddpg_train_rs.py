...
if __name__ == "__main__":
    ...
    parser.add_argument("--write_summary", action="store_true",
                        help="Write trial_summary.json with best val_last_best_reward")
    ...
    if args.checkpoint_folder is not None:
        ...
        key = jax.random.PRNGKey(args.seed)  # or args.seed
        key, init_key = jax.random.split(key)
        ddpg_state = ddpg_agent.init(init_key)
    ...
    def collect_episodes_batched(task_family,
        ...
        def collect_single_episode(problem, key):
            opt_fn = heatmap_optimizer.opt_fn(actor_params)   # use current actor
            ...
            actions = traj['action']      # (T, n, k)
            stats = traj['stats']         # (T, ...) dict
        ...
        return (states_flat, actions_flat, rewards_flat, next_states_flat, dones_flat, timesteps, stats_flat), avg_reward
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id) as mlflow_cm, disable_jit(args.disable_jit) as jit_cm:
        ...
        if args.trial_id:
            mlflow.set_tag("trial_id", args.trial_id)
        ...
        for i in tqdm.tqdm(range(args.outer_train_steps), disable=args.disable_tqdm):
            ...
            batch, avg_rew = collect_episodes_batched(
            ...
            if i % args.log_steps == 0:
                ...
                extra_metrics = {
                    'train_avg_reward': -float(avg_rew),
        ...
        # write trial summary for random search (subprocess-friendly)
        if args.write_summary:
            best_score = float(aggregates["val_last_best_reward"])
            summary = {
                "trial_id": args.trial_id,
                "seed": int(args.seed),
                "best_val_last_best_reward": best_score,
                "mngr_best_step": int(mngr.best_step()),
                "timestamp": time.time(),
                "args": {k: (str(v) if k == "subset" else v) for k, v in vars(args).items()},
            }
            out_path = os.path.join(args.model_save_path, "trial_summary.json")
            with open(out_path, "w") as f:
                json.dump(summary, f, indent=2)
            print(f"[trial_summary] wrote {out_path}", flush=True)
        ...
        if args.ood_path is not None:
            ...
            ood_family = TspTaskFamily(
                ood_size, args.task_batch_size, args.k,
                baseline=args.baseline, causal=args.causal,
                meta_loss_type=args.meta_loss_type, top_k=args.top_k,
                two_opt_t_max=args.two_opt_t_max, first_accept=args.first_accept
            )
            key, subkey = jax.random.split(key)
