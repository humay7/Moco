...
class DDPGAgent:
    ...
    def update_critic(self,
        ...
        # Debug logging for target Q-values statistics
        ...
        def mse_loss(params):
            ...
            current_q_values = self._compute_q_values(params, states, actions, timesteps=None)
        ...
        current_q_values_pred = self._compute_q_values(critic_state.params, states, actions, timesteps=None)
    ...
    def update_actor(self,
        ...
        def actor_loss(params):
            ...
            q_values = self._compute_q_values(critic_state.params, states, actions, timesteps=None)
        ...
        return new_actor_state, new_critic_state, actor_loss_value, actor_grad_norm
    ...
    def _compute_q_values(self, critic_params: Any, states: Any, actions: Any, timesteps: Any = None) -> Any:
        """Compute Q-values using the critic update network.
        
        Uses only the update network for all timesteps. States from lopt already
        have t=0 optimizer features set to zero.t
    ...
    def _concatenate_action_to_state(self, states: Any, actions: Any) -> Any:
        ...
        return states._replace(edges=new_edge_features)
    ...
    def update(self, state: DDPGState, batch: Any, train_actor: bool = True, train_critic: bool = True) -> Tuple[DDPGState, Any]:
        ...
        if train_actor:
            actor_state, critic_state, actor_loss, actor_grad_norm = self.update_actor(
                actor_state,
                critic_state,
                states,
                timesteps
        ...
        actor_param_norm = optax.global_norm(actor_state.params)
        critic_param_norm = optax.global_norm(critic_state.params)

        metrics = {
            'critic_loss': critic_loss,
            'actor_loss': actor_loss,
            'q_values': q_values,
            'actor_param_norm': actor_param_norm,
